  ...
  const debugTransportMarkers = () => {
    ...
    sceneRef.current.traverse(object => {
      ...
      if ((object instanceof THREE.Mesh || object instanceof THREE.Line) && 
          object.position.y > 5 && object.renderOrder >= 1000) {
    ...
    if (potentialMarkers.length === 0) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        marker.position.set(i * 20 - 40, 20, 0); // Position high above the center
        marker.renderOrder = 3000; // Super high render order
        
        sceneRef.current.add(marker);
        console.log(`Created emergency marker at position: ${i * 20 - 40}, 20, 0`);
      }
      ...
      if (polygonRendererRef.current) {
        ...
        const renderer = polygonRendererRef.current as any;
        if (typeof renderer.forceCreateBridgeAndDockPoints === 'function') {
          renderer.forceCreateBridgeAndDockPoints();
        } else {
          console.log("forceCreateBridgeAndDockPoints method not found");
        }
      }
    }
