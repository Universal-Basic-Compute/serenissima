...
export default function WalletButton({ className = '', onSettingsClick }: WalletButtonProps) {
  ...
  return (
    <>
      ...
      {(() => {
        if (isConnected && citizenProfile && citizenProfile.username) { // Cas 1: Connecté, profil complet
          return (
        ...
        } else if (isConnected && citizenProfile && !citizenProfile.username) { // Cas 2: Connecté, profil incomplet
          return (
            <button
        ...
        } else if (isConnected && !citizenProfile) { // Cas 3: Connecté, mais profil en cours de chargement ou introuvable
          return (
            <div className={`${className} px-4 py-2 rounded shadow bg-gray-200 text-gray-700`}>
              Loading Profile...
            </div>
          );
        } else { // Cas 4: Non connecté
          return (
            <button 
              onClick={connectWallet}
              className={`bg-white px-4 py-2 rounded shadow hover:bg-purple-100 transition-colors ${className}`}
