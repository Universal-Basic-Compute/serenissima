...
export class NavigationService {
  ...
  /**
   * Set polygons and build the basic navigation graph
   */
  ...
  /**
   * Build the basic navigation graph from polygon data
   */
  ...
  /**
   * Find the shortest path between two polygons using A* algorithm
   */
  public findShortestPath(start: string, end: string): string[] {
    // Try using the enhanced navigation graph first if available
    if (this.landNavigationGraph && this.landNavigationGraph.enhanced && 
        Object.keys(this.landNavigationGraph.enhanced).length > 0) {
      return this.findPathBetweenPolygons(start, end);
    }
    
    // Fall back to basic A* if enhanced graph is not available
    if (start === end) {
      return [start];
    }
  ...
  /**
   * Get the node position for visualization
   */
   ...
   */
  private createFallbackLandNavigationGraph(): NavigationGraphData {
    console.log('Creating fallback land navigation graph');
    const fallbackGraph: NavigationGraphData = {
   ...
   * Create a minimal fallback water navigation graph
   */
  private createFallbackWaterNavigationGraph(): NavigationGraphData {
