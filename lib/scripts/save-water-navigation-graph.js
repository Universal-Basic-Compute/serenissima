  ...
  polygonFiles.forEach(file => {
    ...
    if (polygon && polygon.id) {
      ...
      if (polygon.canalPoints && Array.isArray(polygon.canalPoints) && polygon.canalPoints.length > 0) {
        polygon.canalPoints.forEach((canalPoint, index) => {
          if (canalPoint.water && canalPoint.edge) {
            allDocks.push({
              id: `dock-${polygon.id}-${index}`,
              polygonId: polygon.id,
              position: canalPoint.water, // Use water point for navigation
              edge: canalPoint.edge,      // Store edge point for reference
              index,
