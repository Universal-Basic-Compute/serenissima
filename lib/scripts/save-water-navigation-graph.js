...
function buildWaterNavigationGraph() {
  ...
  // Second pass: Connect all docks to each other with distances
  // Each dock can connect to any other dock via water
  for (let i = 0; i < allDocks.length; i++) {
    ...
    for (let j = 0; j < allDocks.length; j++) {
      ...
      const distance = calculateDistance(dock1.position, dock2.position);
  ...
  Object.keys(navigationGraph).forEach(dockId => {
    totalConnections += navigationGraph[dockId].length;
  });
