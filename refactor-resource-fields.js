...
function processResourceFile(filePath) {
  try {
    ...
    // Merge lore into historicalNotes if both exist
    if (resource.lore && resource.historicalNotes) {
      resource.historicalNotes = `${resource.historicalNotes}\n\n${resource.lore}`;
    } else if (resource.lore) {
      // If only lore exists, use it as historicalNotes
      resource.historicalNotes = resource.lore;
    }
