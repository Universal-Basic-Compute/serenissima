...
def delegate_businesses_logic(tables: Dict[str, Table], dry_run: bool = False):
    ...
    for overburdened_ai_info in overburdened_ais:
        original_owner_username = overburdened_ai_info['username']
        log.info(f"Processing {original_owner_username}, who runs {overburdened_ai_info['current_business_count']} businesses and needs to delegate {len(overburdened_ai_info['businesses_to_delegate'])}.")

        for business_to_delegate in overburdened_ai_info['businesses_to_delegate']:
