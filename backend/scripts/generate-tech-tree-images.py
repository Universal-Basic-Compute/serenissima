...
async def generate_image_prompt(node):
    try:
        ...
        print(f"Generating prompt for node: {node_title}")
        ...
        3. Be optimized for a square image (128x128 pixels)
        ...
        print(f"Generated prompt for {node_title}: {image_prompt[:100]}...")
        
        return image_prompt
    except Exception as error:
        print(f"Error generating prompt for {node['id']}: {error}")
