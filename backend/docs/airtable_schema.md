...
-   `TargetAmount` (Nombre): Quantité totale de ressource pour ce contrat (remplace `HourlyAmount`). Pour `public_storage`, peut être une très grande valeur. Pour `building_bid`, généralement 1 (pour le bâtiment).
