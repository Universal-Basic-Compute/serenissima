...
*   **Active Contracts**:
    *   Adds **`(PricePerResource * HourlyAmount) / 100`** to `TrustScore` for each contract where the two citizens are `Buyer` and `Seller` (or vice-versa) and `CONTRACTS.EndAt` is in the future and `CONTRACTS.Status` is "active".
