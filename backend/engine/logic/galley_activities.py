...
def process_final_deliveries_from_galley(
    ...
    current_time_venice = now_utc_dt.astimezone(VENICE_TIMEZONE_FINAL_DELIVERY) # Convert UTC to Venice time
...
def process_galley_unloading_activities(
    ...
    current_time_venice_gu = now_utc_dt.astimezone(VENICE_TIMEZONE_GALLEY_UNLOAD) # Convert UTC to Venice time
    ...
    try:
        ...
        for galley_record in arrived_galleys:
            ...
            for contract_to_fetch in pending_import_contracts:
                ...
                amount = float(contract_to_fetch['fields'].get('HourlyAmount', 0)) 
                ...
                if not idle_citizens: 
                    ...
                    break
                citizen_for_task = idle_citizens.pop(0) 
