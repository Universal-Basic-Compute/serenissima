...
def create_transaction_record(tables, from_user: str, to_user: str, amount: float, business_id: str) -> Optional[Dict]:
    """Create a transaction record for a wage payment."""
    log.info(f"Creating transaction record for wage payment: {from_user} -> {to_user}, amount: {amount}")
    ...
    try:
        ...
        transaction = tables['transactions'].create({
            ...
            "Seller": from_user,  # Business owner is the seller (paying)
            "Buyer": to_user,     # Citizen is the buyer (receiving)
            "Price": amount,
            "CreatedAt": now,
            "UpdatedAt": now,
            "ExecutedAt": now,
