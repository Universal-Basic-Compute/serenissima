...
async def get_contracts_handler(
    ...
    limit: Optional[int] = None # Optional limit for records
):
    """
    Get contracts from Airtable based on query parameters.
    Used by LandDetailsPanel to fetch land listings and offers.
    """
    try:
        log_header(f"Fetching contracts with filters: Asset={Asset}, AssetType={AssetType}, Status={Status}, Type={Type}, Buyer={Buyer}, Seller={Seller}", color_code=LogColors.INFO)
        
        formula_parts = []
        if Asset:
            formula_parts.append(f"{{Asset}}='{_escape_airtable_value(Asset)}'")
        if AssetType:
            formula_parts.append(f"{{AssetType}}='{_escape_airtable_value(AssetType)}'")
        if Status:
            formula_parts.append(f"{{Status}}='{_escape_airtable_value(Status)}'")
        if Type:
            formula_parts.append(f"{{Type}}='{_escape_airtable_value(Type)}'")
        if Buyer:
            # Attempt to find by username or wallet for Buyer
            buyer_citizen = find_citizen_by_identifier(citizens_table, Buyer)
            if buyer_citizen and buyer_citizen['fields'].get('Username'):
                 formula_parts.append(f"{{Buyer}}='{_escape_airtable_value(buyer_citizen['fields']['Username'])}'")
            else: # Fallback to direct match if citizen not found or no username
                formula_parts.append(f"{{Buyer}}='{_escape_airtable_value(Buyer)}'")

        if Seller:
            # Attempt to find by username or wallet for Seller
            seller_citizen = find_citizen_by_identifier(citizens_table, Seller)
            if seller_citizen and seller_citizen['fields'].get('Username'):
                formula_parts.append(f"{{Seller}}='{_escape_airtable_value(seller_citizen['fields']['Username'])}'")
            else: # Fallback to direct match
                formula_parts.append(f"{{Seller}}='{_escape_airtable_value(Seller)}'")

        formula = None
        if formula_parts:
            formula = f"AND({', '.join(formula_parts)})"
        
        log.info(f"Airtable formula for contracts: {formula}")
        
        query_options = {"formula": formula}
        if limit is not None:
            query_options["max_records"] = limit
            
        records = contracts_table.all(**query_options, sort=[('-CreatedAt')]) # Sort by most recent
        
        contracts_response = []
        for record in records:
            fields = record['fields']
            # Ensure ContractId is present, otherwise use Airtable record ID as fallback for 'id'
            contract_id_val = fields.get("ContractId", record['id'])

            # Attempt to get SellerName and BuyerName
            seller_name = fields.get("Seller") # Default to username
            if fields.get("Seller"):
                seller_profile = find_citizen_by_identifier(citizens_table, fields.get("Seller"))
                if seller_profile:
                    seller_name = f"{seller_profile['fields'].get('FirstName', '')} {seller_profile['fields'].get('LastName', '')}".strip()
                    if not seller_name: # Fallback if names are empty
                        seller_name = seller_profile['fields'].get('Username', fields.get("Seller"))
            
            buyer_name = fields.get("Buyer") # Default to username
            if fields.get("Buyer"):
                buyer_profile = find_citizen_by_identifier(citizens_table, fields.get("Buyer"))
                if buyer_profile:
                    buyer_name = f"{buyer_profile['fields'].get('FirstName', '')} {buyer_profile['fields'].get('LastName', '')}".strip()
                    if not buyer_name: # Fallback if names are empty
                        buyer_name = buyer_profile['fields'].get('Username', fields.get("Buyer"))

            contract_data = {
                "id": contract_id_val, # Use ContractId as 'id' for frontend
                "airtableRecordId": record['id'], # Keep Airtable record ID for debugging or direct updates
                "Type": fields.get("Type"),
                "Asset": fields.get("Asset", fields.get("ResourceType")), # Fallback to ResourceType for Asset
                "AssetType": fields.get("AssetType"),
                "Seller": fields.get("Seller"),
                "SellerName": seller_name,
                "Buyer": fields.get("Buyer"),
                "BuyerName": buyer_name,
                "PricePerResource": fields.get("PricePerResource"),
                "TargetAmount": fields.get("TargetAmount"),
                "Status": fields.get("Status"),
                "Notes": fields.get("Notes"),
                "CreatedAt": fields.get("CreatedAt"),
                "UpdatedAt": fields.get("UpdatedAt"),
                "ExecutedAt": fields.get("ExecutedAt"),
                "Title": fields.get("Title"),
                "Description": fields.get("Description"),
                # Add any other fields the frontend might need
            }
            contracts_response.append(contract_data)
        
        log.info(f"Found {len(contracts_response)} contracts matching criteria.")
        return {"success": True, "contracts": contracts_response}
        
    except Exception as e:
        error_msg = f"Failed to get contracts: {str(e)}"
        log.error(f"{LogColors.FAIL}{error_msg}{LogColors.ENDC}", exc_info=True)
        raise HTTPException(status_code=500, detail=error_msg)
